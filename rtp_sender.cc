...
RTPSender::RTPSender(
    ...
    : clock_(clock),
	  ...
	  circuit_breaker_((audio || (this ->VideoCodecType() != kRtpVideoVp8
							  &&  this ->VideoCodecType() != kRtpVideoVp9
							  &&  this ->VideoCodecType() != kRtpVideoH264)) ? nullptr : new CircuitBreaker(this, clock)),
...
void RTPSender::PushRtcpIntervalToCb(uint32_t rtcp_interval) {
	if (circuit_breaker_) {
		circuit_breaker_->SetRtcpInterval(rtcp_interval);
	}
}
...
void RTPSender::OnReceivedRtcpReportBlocks(
	...
	if (circuit_breaker_) {
		circuit_breaker_->OnReceivedRtcpReportBlocks(report_blocks, receiver_estimated_max_bitrate_bps);
	}
